...
- 서비스(`agenthook.py`)는 stdlib만 사용 — `pip install` 불필요. 실행에는 `claude` CLI(Claude Code)만 필요.
- 설정 TUI(`tui/`)는 [ratatui](https://ratatui.rs) 기반 Rust 바이너리.
...
> `BIN_DIR=~/bin ./install.sh` 로 TUI 설치 위치를 바꿀 수 있다.
...
`routes.json` 을 손으로 고치는 대신, 엔드포인트를 추가하고 프롬프트(어떤 작업을
시킬지)를 작성할 수 있는 ratatui TUI. 빌드는 `install.sh` 또는 직접:

```bash
cargo build --release --manifest-path tui/Cargo.toml
agenthook-tui [routes.json]           # 기본: $AGENTHOOK_CONFIG 또는 ./routes.json
```
...
```bash
cp routes.example.json routes.json && chmod 600 routes.json   # 시크릿 채우기
